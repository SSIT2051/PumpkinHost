...
class MinecraftNetworkBridge(
    ...
    private suspend fun runJavaTcpListener(config: ServerConfig) {
        ...
        try {
            javaServerSocket = ServerSocket(port, 50, InetAddress.getByName("0.0.0.0"))
            onLog(
                LogLevel.INFO,
                "tokio::net",
    ...
    private suspend fun runBedrockUdpListener(config: ServerConfig) {
        ...
        try {
            bedrockDatagramSocket = DatagramSocket(bedrockPort, InetAddress.getByName("0.0.0.0"))
            bedrockDatagramSocket?.reuseAddress = true
            onLog(
                LogLevel.INFO,
                "geyser::bedrock",
